...
def comments(path: Path, lang: str, config: Path | None = None) -> list[tuple[int, int, str]]:
    ...
    return [
        ...
        for h in mutants.kind_hits(path, lang, "comment", config)
    ]


def _pre_image_comments(
    repo: Repo, rel: str, lang: str, staged: bool, config: Path | None
) -> Counter[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="mutation-gate-pre-") as tmp:
        ...
        return Counter(t for _, _, t in comments(copy, lang, config))
...
def check(repo: Repo, changed: dict[str, set[int]], wvs, staged: bool) -> list[Finding]:
    ...
    for rel, lines in sorted(changed.items()):
        ...
        budget = _pre_image_comments(repo, rel, lang, staged, config)
