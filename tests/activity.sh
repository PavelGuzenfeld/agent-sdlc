...
check_not_contains() {
    name="$1"; haystack="$2"; needle="$3"
    case "$haystack" in
        *"$needle"*) echo "FAIL: $name (expected NOT to find: $needle)" >&2; failures=$((failures + 1)) ;;
        *) ;;
    esac
}
...
make_repo "$fake_home" workspace work-repo "Teammate Person" "teammate-noaddr" "add a teammate's feature" "2026-03-05T11:00:00"
...
check_contains "default: derives author from git config, finds work commit" \
    "$out" "work${tab}work-repo${tab}add work feature"
check_contains "default: derives author from git config, finds personal commit" \
    "$out" "personal${tab}home-repo${tab}fix personal bug"
check_not_contains "default: a teammate's commit under the same repo is excluded" \
    "$out" "add a teammate's feature"
...
check_contains "sessions block marks the personalspace session personal under a non-pavelgu HOME" \
    "$personal_session_line" "${tab}personal${tab}"
...
check_contains "ACTIVITY_PERSONAL_DIRS/ACTIVITY_AUTHOR override picks up a custom layout" \
